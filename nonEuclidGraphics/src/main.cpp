...
int main()
{
    ...
    tex->SetTextureImage();
    // 添加网格
    pworld->AddObj(std::make_shared<Mesh>("../data/ball.obj", tex), {PI<float>,0.f,0.f }, { 0.05f, 0.05f, 0.05f }, matf3::Identity());
    for(int i = -1; i <=1; i+=2)
        for (int j = -1; j <= 1; j += 2)
            for (int k = -1; k <= 1; k += 2)
                pworld->AddObj(std::make_shared<Mesh>("../data/ball.obj", tex, Material::MaterialType::COPPER), { (float)i/2.f + PI<float>, (float)j/2.f, (float)k/2.f }, {0.05f, 0.05f, 0.05f}, matf3::Identity());
